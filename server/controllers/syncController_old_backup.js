...
/**
 * 批量同步任务（上传本地变更 + 下载服务器变更）
 */
export async function syncTasks(req, res) {
  const userId = req.userId;
  const {
    device_id,
    last_sync_at, // 客户端最后同步时间
    tasks = [], // 客户端要上传的任务
    deleted_task_ids = [], // 客户端删除的任务ID
    lists = [], // 列表数据
    deleted_list_ids = [],
    tags = [], // 标签数据
    deleted_tag_ids = []
  ...
  try {
    ...
    if (deleted_task_ids.length > 0) {
      ...
      syncResult.deleted_tasks = result.affectedRows;
    }
    ...
    const serverTasks = await query(downloadSql, downloadParams);
    syncResult.downloaded_tasks = serverTasks.map(formatTaskResponse);
    ...
    const serverLists = await query(listDownloadSql, listDownloadParams);
    syncResult.downloaded_lists = serverLists.map(formatListResponse);
    ...
    const serverTags = await query(tagDownloadSql, tagDownloadParams);
    syncResult.downloaded_tags = serverTags.map(formatTagResponse);

    // 10. 记录同步日志
    await query(
      'INSERT INTO sync_logs (user_id, device_id, sync_type, records_count, status) VALUES (?, ?, ?, ?, ?)',
      [userId, device_id, 'full', syncResult.uploaded_tasks + syncResult.uploaded_lists + syncResult.uploaded_tags, 'success']
    );
